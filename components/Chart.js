...
const Chart = ({ currentPrice, logoUrl, name, symbol, priceChangePercentage7d, sparkline }) => {
  ...
  const formatUSD = value => {
    'worklet';
    if (value === '') {
      const formattedValue = `₹${latestCurrentPrice.value.toFixed(2).toLocaleString('en-US', { currency: 'INR' })}`
      return formattedValue;
    }

    const formattedValue =`₹${parseFloat(value*76).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`
    return formattedValue;
  };
